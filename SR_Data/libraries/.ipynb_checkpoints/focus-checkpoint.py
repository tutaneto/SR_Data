...
def focus_load_data():
    global dff, dff_old
    if type(dff) == int:
        dff = pd.read_csv('data/focus.csv', skipinitialspace=True, compression='zip')
        dff_old = dff.copy()
        focus_adjust_data()
    ...
    for i in range(4):
        ...
        for pos in range(2):
            ...
            add_image(fig, img,x1+FOCUS_DATA_X[pos]-24, y1+FOCUS_DATA_Y2,
                FOCUS_ARROW_W, FOCUS_ARROW_H, xanchor='right', yanchor='middle')
...
def show_focus_data(fig, annot, focus_year, gtype=None, debug=None):
    ...
    for i in range(len(vars)):
        ...
        add_image(fig, img, tit_x - 52, tit_y,
                FOCUS_ICON_W, FOCUS_ICON_H, xanchor='right', yanchor='middle')
