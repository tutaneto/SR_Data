...
def download_USD_BRL_Bacen():
    ...
    df = df.append(df_new[df_new.dataHoraCotacao > last_date_ok])
...
def load_USD_BRL():
    ...
    df_us['Date'] = df_us['Date'].astype(np.datetime64)
    ...
    # df_us['data'] = df_us.dataHoraCotacao.str.slice(0,10).astype(np.datetime64)
...
def load_ibov():
    ...
    df_bo['date'] = df_bo['date'].astype(np.datetime64)
    ...
    df_bo = df_bo.append(df_new.loc[df_new.index > last_date_ok])
...
def load_nasdaq():
    ...
    df_nq['Date'] = df_nq['Date'].astype(np.datetime64)
...
def load_cdi(only_cdi=False):
    ...
    df_cdi['date'] = df_cdi['date'].astype(np.datetime64)
