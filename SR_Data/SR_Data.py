...
def main(args=None):
    global symbol_old, time_start, pos_out, file_name_in
    ...
    if MERC_FIN_AUTO_ROBO:
        ...
        # Initialize server manager with default paths
        server = ServerManager()
        server.set_status("Starting")
            ...
            try:
                ...
                if datetime.now() - last_health_check > health_check_interval:
                    if not server.check_health():
                        server.set_status("Error", error="Health check failed")
